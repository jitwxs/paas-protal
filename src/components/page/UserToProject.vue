<template>
    <div class="usertoproject">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-tickets"></i>{{userId}}的项目</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
    ...
    export default {
        ...
        methods:{
            ...
            getUserProject:function () {
                this.$axios.get('/project/list'+ "?userId=" + this.userId  +  "&current=" + this.currentPage + "&size=5")
                    .then(response=>{
            ...
            search:function () {
                this.$axios.get('/project/list'+ "?id=" + this.select_projectId + "&name=" +  this.select_projectName +  "&current=" + this.currentPage + "&size=5")
                    .then(response=>{
                        console.log(response)
                        this.projectInfo = response.data.data.records;
                        this.totalCount = response.data.data.total;
            ...
            handleCurrentChange:function (val) {
                this.$axios.get('/project/list'+ "?userId=" + this.userId  +  "&current=" + val + "&size=5")
                    .then(response=>{
                        if(response.data.code == 0) {
                            this.projectInfo = response.data.data.records;
                        } else {
